...
ubxResPartPacket resPartPacket;
ubxDataPartPacket dataPartPacket;
...
int ubx_data_parse(ubxDATA *ubxData)
{
    int ret = -1;
    ...
    ubx_parse_pack(buffer, dataLen, ubxData);

    char date[64] = {0};
    if (ubxData->timeValid)
    {
        tempTime = ubx_utc_local(&(ubxData->time), date);
    }
        
    if (tt != tempTime
        && tempTime > MIN_TIME_STAMP
        && ubxData->latlonValid == 1
        && ubxData->timeValid == 1
        && ubxData->lat > 0
        && ubxData->lon > 0
        && (ubxData->fixtype == 0x02
        || ubxData->fixtype == 0x03))
    {
        printf("%d, %lf, %lf, %s %s %d\r\n", ubxData->fixtype, ubxData->lat, ubxData->lon, date,
            __FUNCTION__, __LINE__);
        tt = tempTime;
        err = 0;
    ...
    else
    {
                ...
                && ubxData->fixtype != 0x03))
        {
            ...
            if (err > 10)
            {
                printf("%d, %lf, %lf, %s %s %d \r\n", ubxData->fixtype, ubxData->lat, 
                    ubxData->lon, date, __FUNCTION__, __LINE__);
                memset(ubxData, 0, sizeof(ubxDATA));
                err = 0;
            }
...
void *nmea_parse_thread(void *arg)
{
    ...
    memset(&resPartPacket, 0, sizeof(ubxResPartPacket));
    memset(&dataPartPacket, 0, sizeof(ubxDataPartPacket));
...
void *nmea_cfg_thread(void *arg)
{
    ...
    while (1)
    {
        if (fd >= 0 && gCmdResult == 1 && time(NULL) - stime >= 300)
        {
            oldType = dataType;
            dataType = 1;
            if (oldType == 0)

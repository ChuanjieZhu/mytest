/*
*******************************************************************************
**  Copyright (c) 2011, 深圳市飞瑞斯科技有限公司
**  All rights reserved.
**	文件名:gbk.c
**  文件说明: 将GBK编码转换为Unicode编码或BIG5编码
**  创建日期: 2011.09.22
**
**  当前版本：1.0
**  作者：Kendy
...
/************************************
函数名称：GBKToUnicode
函数功能: 将GBK编码的字符串转换为Unicode编码的字符串
入口参数：to ,Unicode编码输出的指针,from, GBK编码输入的指针,bytes,输入字符串的长度
返回值  ：
		unicode 编码的个数.
		-1: 失败
************************************/
int GBKToUnicode(unsigned short *to, char *from, int bytes, int *piBytesLeft)
{
	int words;
	unsigned short ch1, ch2;
	const convert_table_t *index;
...
/************************************
函数名称：UnicodeToUTF_8
函数功能: 将Unicode编码的字符串转换为UTF_8编码的字符串
入口参数：to ,UTF_8编码输出的指针,from, Unicode编码输入的指针,count,输入Unicode编码的个数
返回值  ：
		0: 成功
		-1: 失败
************************************/
int UnicodeToUTF_8(char* to, unsigned short* from, int count)
{
	...
	for(i = 0; i < count; i++)
	{
		...
		else if(*(from+i) < 0x0800)
		{
			pchar = (char *)(from+i);
			to[0] = (0xC0 | ((pchar[1] & 0xF8) >> 3));
			to[1] = (0x80 | ((pchar[1] & 0x07) << 3) | (pchar[0] & 0x07));
			to += 2;
		}
		...
		{
			pchar = (char *)(from+i);
			to[0] = (0xE0 | ((pchar[1] & 0xF0) >> 4));
			to[1] = (0x80 | ((pchar[1] & 0x0F) << 2)) + ((pchar[0] & 0xC0) >> 6);
			to[2] = (0x80 | (pchar[0] & 0x3F));
			to += 3; 
		}
	}
	to[0] = '\0';
...
/************************************
函数名称：CheckUtf8NameLen
函数功能: 获取utf8格式字符串中的数目
入口参数：in : 输入字符串
返回值  ：
		0
************************************/
int CheckUtf8NameLen(char *in, int * piSingleCharNum, int * piDoubleCharNum, int * piThreeCharNum)
{
    char *pSrc = in;
...
/************************************
函数名称：UTF_8ToUnicode
函数功能: 将UTF_8编码的字符串转换为Unicode编码的字符串
入口参数：to ,UTF_8编码输出的指针,from, Unicode编码输入的指针,count,输入Unicode编码的个数
返回值  ：
		0: 成功
		-1: 失败
************************************/
int UTF_8ToUnicode(char *in, wchar_t **out, int *outsize)
{
    char *pSrc = in;
    wchar_t *result = NULL;
    int resultsize = 0;
	...
	/* should be enough */
    result = (wchar_t *)malloc(strlen(in) * 2 + 2);
...
/************************************
函数名称：UnicodeToGBK
函数功能: 将Unicode编码的字符串转换为GBK编码的字符串
入口参数：to ,GBK编码输出的指针,from, Unicode编码输入的指针,words,输入Unicode编码的个数
返回值  ：
		GBK 编码的长度.
		-1: 失败
************************************/	
int UnicodeToGBK(char *to, unsigned short *from, int words)
{
	int bytes = 0;
	unsigned short v;
	const convert_table_t *index;
...
/************************************
函数名称：GBKFileToBIG5File
函数功能: 将GBK编码的文件转换为BIG5编码的文件
入口参数：to ,BIG5编码的文件名,from, GBK编码的文件名
返回值  ：
		0: 成功
		-1: 失败
************************************/	
int GBKFileToBIG5File(char* from, char* to)
{
	FILE* srcfp = NULL;
	FILE* mapfp = NULL;
	int destfp = 0;
	char gbkBuf[1024 + 4] = {0};
	char utf8Buf[2048] = {0};
	char big5Buf[2048] = {0};
	unsigned short unicodeBuf[1024] = {0};
	char ch1,ch2,ch3;
	int offset;
	int count = 0;
	int len = 0;
	...
	mapfp = fopen( "/root/res/unicode-big5.tab", "r" );
	if(mapfp == NULL)
	{
		TRACE("fopen /root/res/unicode-big5.tab fail!!\r\n");
		fclose(srcfp);
		return -1;
	}

	destfp = open(to, O_CREAT|O_WRONLY|O_TRUNC, 0777);
	if(destfp == -1) {
		TRACE("create %s fail!!\r\n", to);
		fclose(srcfp);
		fclose(mapfp);
	...
	while(fgets(gbkBuf + iGbkLeft, 1024 - iGbkLeft, srcfp))
	{
		len = strlen(gbkBuf);
		
		memset(unicodeBuf, 0, sizeof(unicodeBuf));
		memset(utf8Buf, 0, sizeof(utf8Buf));
		memset(big5Buf, 0, sizeof(big5Buf));
		
		count = GBKToUnicode(unicodeBuf, gbkBuf, len, &iGbkLeft); 
		
		if (iGbkLeft > 0)
		{
			strncpy(gbkBuf, gbkBuf + len - iGbkLeft, iGbkLeft);
		}
		
		UnicodeToUTF_8(utf8Buf,unicodeBuf,count);	
		len = strlen(utf8Buf);
		x = 0;
		for( i = 0 ; i < len ; i++ )
		{		
			...
			{
				ch1 -= 0xE0;
				ch2 = utf8Buf[i+1] - 0x80;
				ch3 = utf8Buf[i+2] - 0x80;
				i += 2;
				offset = ch1 * 4096 + ch2 * 64 + ch3 ;
				fseek( mapfp, offset * 2, SEEK_SET);
				fread( big5Buf+x, 2, 1, mapfp );
				x += 2;
			}
			else if( ch1 >= 0xC0 )
			{
		...
		big5Buf[x] = '\0';
		write(destfp, big5Buf, strlen(big5Buf));
	}
...
/******************************************************************************
 * 函数名称： GBKBufToBIG5Buf
 * 功能： GBK编码字符串转换BIG5编码字符串
 * 参数： pcFrom GBK字符串; iLen字符串长度;  pcTo BIG编码字符串
 * 返回： 返回BIG5编码字符串长度
 * 创建作者： LPH
 * 创建日期： 2012-10-26
 * 修改作者：
 * 修改日期：
 ******************************************************************************/
int GBKBufToBIG5Buf(char *pcFrom, int iLen, char *pcTo)
{
	FILE* mapfp = NULL;
	char gbkBuf[1028] = {0};
	char utf8Buf[1028] = {0};
	char big5Buf[1028] = {0};
	unsigned short unicodeBuf[512] = {0};
	char ch1 = 0x00;
	char ch2 = 0x00;
	char ch3 = 0x00;
	int offset = 0;
	...
	int iFromOffset = 0;	//GBK偏移
	int iToOffset = 0;		//BIG5偏移
	...
	{	
		...
		if(iLen >= 1024)
		{
			memcpy(gbkBuf, caGbkLeft, iGbkLeftLen);
			memcpy(gbkBuf + iGbkLeftLen, pcFrom + iFromOffset, 1024 - iGbkLeftLen);
			iLen -= 1024 - iGbkLeftLen;
			iFromOffset += 1024 - iGbkLeftLen;
		}
		else
		{
			memcpy(gbkBuf, caGbkLeft, iGbkLeftLen);
			memcpy(gbkBuf + iGbkLeftLen, pcFrom + iFromOffset, iLen);
			iLen -= iLen;
		}
			
		len = strlen(gbkBuf);
		count = GBKToUnicode(unicodeBuf, gbkBuf, len, &iGbkLeftLen); 

		if (iGbkLeftLen > 0)
		{
			memset(caGbkLeft, 0, sizeof(caGbkLeft));
			strncpy(caGbkLeft, gbkBuf + len - iGbkLeftLen, iGbkLeftLen);
		}
		UnicodeToUTF_8(utf8Buf, unicodeBuf, count);	
		len = strlen(utf8Buf);
		x = 0;
		...
		big5Buf[x] = '\0';
		memcpy(pcTo + iToOffset, big5Buf, strlen(big5Buf));
		iToOffset += strlen(big5Buf);
	}
...
/****************************************************************************** 
* 函数名称： GBKBufToUTF8Buf 
* 功能： GBK编码字符串转换UTF8编码字符串 
* 参数： pcFrom GBK字符串; iLen字符串长度; pcTo BIG编码字符串 
* 返回： 返回BIG5编码字符串长度 
* 创建作者： LPH 
* 创建日期： 2012-10-26 
...
* 修改日期： 
******************************************************************************/ 
int GBKBufToUTF8Buf(char *pcFrom, int iLen, char *pcTo) 
{ 
    char gbkBuf[1028] = {0}; 
    char utf8Buf[1028] = {0}; 
    unsigned short unicodeBuf[512] = {0}; 
...
wchar_t * ZhcnToUincode(char* strTemp)
{
	int words;
	unsigned short ch1, ch2;
	const convert_table_t *index;
	static wchar_t zhcn_uni[4096] = {0};
	wchar_t *to = zhcn_uni;
	...
	int bytes = strlen(strTemp);
		
	words = 0;
	while (bytes > 0)
	{
		ch2 = *(unsigned char *)strTemp++;
		if (ch2 > 0x80)
		{
			if (bytes > 1)
			{

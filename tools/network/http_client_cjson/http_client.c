...
static MRJEcode connect_nnob(mrj_socket_t sockfd, const struct sockaddr *saptr, socklen_t salen, long nsec)
{
    ...
    {
        TRACE("> connect nnob fcntl fail(%s). %s %d\r\n", STR_ERRNO, MDL);
...
static MRJEcode recv_data_download(struct SessionHandle *data)
{
    MRJEcode res = MRJE_UNKNOWN;
    size_t per_bytes = 0;
    size_t already_bytes = 0;
    ...
    do {
        ...
        if (per_bytes > 0) 
        {
            ...
            {
                write_bytes = data->set.fwrite_func(recv_buff, 1, per_bytes, data->set.event_args);
                if (write_bytes != per_bytes) 
                {
                    res = MRJE_WRITE_ERROR;
                    break;
                }
            }
...
static MRJEcode send_hcmd(struct SessionHandle *data, const char *hcmd, size_t hlen)
{
	MRJEcode res = MRJE_UNKNOWN;
	...
	if ( res != MRJE_OK ) 
    {
        TRACE("> send hcmd connect host error. %s %d\r\n", MDL);
        return res;
	}

    if (data->set.verbose)
    {
        show_req_head(hcmd, hlen);
    }
    
	res = send_data(data, hcmd, hlen);
    if ( res != MRJE_OK ) 
    {
...
static MRJEcode parse_url(struct SessionHandle *data)
{
	...
	if (!strncasecmp(data->set.url, "http://", 7))
	{
        itr = 7;
	}
	else if (!strncasecmp(data->set.url, "https://", 8))
	{
        itr = 8;    
	}
    else
    {
        TRACE("> parse url fail, url(%s). %s %d\r\n", data->set.url, MDL);
...
static MRJEcode http_setopt(struct SessionHandle *data, MRJHTTPoption option, va_list param)
{
    char *argptr;
...
{   
    ...
    if (data->set.httpreq == HTTPREQ_POST) 
    {
        ...
        snprintf(data->request_send, sizeof(data->request_send) - 1, 
             S_pos_head, data->remote_path, data->host, data_len);
    } 
    else if (data->set.httpreq == HTTPREQ_GET) 
    {
        ...
        {
            snprintf(data->request_send, sizeof(data->request_send) - 1,
                S_get_head, data->remote_path, data->host, data->port);
        }
        else 
        { 
            snprintf(data->request_send, sizeof(data->request_send) - 1,
                S_get_head_range, data->remote_path, data->set.set_resume_from, 
                data->host, data->port);
        }
    } 
    else 
    ...
    head_len = strlen(data->request_send);
    total_len = head_len + data_len + 1;

    char *send = (char *)malloc(total_len);
    if (!send) 
    {
        TRACE("> http_transfer malloc error. %s %d\r\n", MDL);
        return MRJE_OUT_OF_MEMORY;
    }
    ...
    {
        strncpy(send, data->request_send, head_len);
    }
    
    if (data->set.send_data && data_len > 0)
    {
        strncpy(send + head_len, (char *)data->set.send_data, data_len);
    }
    ...
    if (data->set.verbose)
    {
        TRACE("send_hcmd ret: %d(%s) %s %d\r\n", res, http_client_strerror(res), MDL);
    }
    ...
    if (res == MRJE_OK) 
    {
        ...
        else if (data->set.httpreq == HTTPREQ_POST) 
        {
            if (data->info.httpcode == 200) 
            {
                ...
                {
                    size_t data_size = 0;
                    data_size = data->set.fwrite_func(data->set.recv_data, 
                                                      1, 
...
static MRJEcode http_perform(struct SessionHandle *data)
{
    MRJEcode res = MRJE_OK;
    ...
    if (res != MRJE_OK) 
    {
        TRACE("> http perform load url error. %s %d\r\n", MDL);
        return res;
    }
    ...
    if (res != MRJE_OK) 
    {
        TRACE("> http perform transfer error. %s %d\r\n", MDL);
        return res;
    } 
...
{
    return http_perform((struct SessionHandle *)handle);
}

...
cmdResPacket cmdPacket;
dataResPacket dataPacket;
...
int ubx_cmd_res_pack(unsigned char *buffer, int *dataLen)
{
    ...
    *dataLen = 0;
    ...
    if (cmdPacket.len != 0)
    {
        checkOffset = cmdPacket.len;
        memcpy(checkBuf, cmdPacket.buffer, cmdPacket.len);
    ...
    if (dataLen >= 4)
    {
        ...
        {       
                ...
                && buffer[i + 1] == UBX_HEADER_62)
            {
                ...
                /* ack response data len is 10 */
                if (offset + ACK_DATA_LEN > dataLen)
                {
                    packetFlag = 1;
                    break;
                }

                if (buffer[offset + 2] == UBX_CLASS_ACK
                    && (buffer[offset + 3] == UBX_CLASS_ACK_ACK 
                        || buffer[offset + 3] == UBX_CLASS_ACK_NAK)
                    && buffer[offset + 6] == classId
                    && buffer[offset + 7] == messageId)
                {
...
int ubx_cmd_res_part_content(unsigned char *buffer, int checkLen)
{
    ...
    for (i = index; i < checkLen; i++)
    {
        ...
        {
            offset = i;
            cmdPacket.len = checkLen - offset;
            if (cmdPacket.len > ACK_DATA_LEN)
            {
                cmdPacket.len = ACK_DATA_LEN;
            }
            
            memcpy(cmdPacket.buffer, buffer + offset, cmdPacket.len);
...
int ubx_data_pack(unsigned char *buffer, int *dataLen)
{
    ...
    *dataLen = 0;
    ...
    if (dataPacket.len != 0)
    {
        checkOffset = dataPacket.len;
        memcpy(checkBuff, dataPacket.buffer, dataPacket.len);
...
int ubx_data_part_content(unsigned char *buffer, int dataLen)
{
    ...
    for (i = index; i < dataLen; i++)
    {
        ...
        {
            offset = i;
            dataPacket.len = dataLen - offset;
            if (dataPacket.len > UBX_SIGNAL_PACK_LEN)
            {
                dataPacket.len = UBX_SIGNAL_PACK_LEN;
            }

            memcpy(dataPacket.buffer, buffer + offset, dataPacket.len);

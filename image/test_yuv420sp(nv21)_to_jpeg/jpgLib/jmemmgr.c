...
METHODDEF(JSAMPARRAY) alloc_sarray (j_common_ptr cinfo, int pool_id, JDIMENSION samplesperrow, JDIMENSION numrows) /* Allocate a 2-D sample array */
{
	my_mem_ptr mem = (my_mem_ptr) cinfo->mem;
	JSAMPARRAY result;
	JSAMPROW workspace;
...
#else
  if(shdr_ptr != NULL){
	small_pool_ptr tmp_ptr = NULL;
	small_pool_ptr pre_ptr = NULL;
    while (1) {
	  tmp_ptr = pre_ptr = shdr_ptr;
	  while(tmp_ptr->hdr.next != NULL) {
	  	pre_ptr = tmp_ptr;
	  	tmp_ptr=tmp_ptr->hdr.next;
  	  }
      space_freed = tmp_ptr->hdr.bytes_used +
		  tmp_ptr->hdr.bytes_left +
		  SIZEOF(small_pool_hdr);
      jpeg_free_small(cinfo, (void *) tmp_ptr, space_freed);
      mem->total_space_allocated -= space_freed;
	  if(tmp_ptr == pre_ptr) break;
	  pre_ptr->hdr.next = NULL;
    }
	shdr_ptr = NULL;
  }
#endif
